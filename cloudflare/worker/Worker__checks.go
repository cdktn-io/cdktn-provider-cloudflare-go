...
func (w *jsiiProxy_Worker) validatePutTailConsumersParameters(value interface{}) error {
	...
	case *[]*WorkerTailConsumers:
		value := value.(*[]*WorkerTailConsumers)
	...
	case []*WorkerTailConsumers:
		value_ := value.([]*WorkerTailConsumers)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
...
func (j *jsiiProxy_Worker) validateSetLifecycleParameters(val *cdktn.TerraformResourceLifecycle) error {
	if err := _jsii_.ValidateStruct(val, func() string { return "parameter val" }); err != nil {
		return err
	}
