...
func (j *jsiiProxy_WorkersScriptExportsMap) validateSetInternalValueParameters(val interface{}) error {
	...
	case *map[string]*WorkersScriptExports:
		val := val.(*map[string]*WorkersScriptExports)
	...
	case map[string]*WorkersScriptExports:
		val_ := val.(map[string]*WorkersScriptExports)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *map[string]*WorkersScriptExports; received %#v (a %T)", val, val)
		}
	}
