...
func (a *jsiiProxy_AccountMemberPoliciesResourceGroupsList) validateResolveParameters(context cdktf.IResolveContext) error {
	if context == nil {
...
func (j *jsiiProxy_AccountMemberPoliciesResourceGroupsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*AccountMemberPoliciesResourceGroups:
		val := val.(*[]*AccountMemberPoliciesResourceGroups)
	...
	case []*AccountMemberPoliciesResourceGroups:
		val_ := val.([]*AccountMemberPoliciesResourceGroups)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*AccountMemberPoliciesResourceGroups; received %#v (a %T)", val, val)
		}
	}
