...
func (j *jsiiProxy_RulesetRulesActionParametersHeadersMap) validateSetInternalValueParameters(val interface{}) error {
	...
	case *map[string]*RulesetRulesActionParametersHeaders:
		val := val.(*map[string]*RulesetRulesActionParametersHeaders)
	...
	case map[string]*RulesetRulesActionParametersHeaders:
		val_ := val.(map[string]*RulesetRulesActionParametersHeaders)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *map[string]*RulesetRulesActionParametersHeaders; received %#v (a %T)", val, val)
		}
	}
